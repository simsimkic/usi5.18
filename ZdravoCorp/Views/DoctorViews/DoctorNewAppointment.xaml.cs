                ...
                if (rbSurgery.IsChecked == true)
                {
                    ...
                    else
                    {
                        ...
                        if (time.Start >= DateTime.Now && time.End >= time.Start)
                        {
                            ...
                            {
                                Appointment appointment = new Appointment(random.Next(1, 10001),time, doc, mr, Appointment.AppointmentStatus.Booked, type, new AppointmentRespository().AssignRoom(RoomType.OperatingRoom));
                                _scheduleService.NewAppointment(appointment);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (time.Start >= DateTime.Now)
                        {
                            if (_scheduleService.IsAvailable(doc, time) && _scheduleService.IsAvailable(patient, time))
                            {
                                Appointment appointment = new Appointment(random.Next(1, 10001),time, doc, mr, Appointment.AppointmentStatus.Booked, type, new AppointmentRespository().AssignRoom(RoomType.ExaminationRoom));
                                _scheduleService.NewAppointment(appointment);
                                MessageBox.Show("Appointment created.");
                            }
                            else
